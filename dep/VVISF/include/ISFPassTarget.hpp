...
/*!
\ingroup VVISF_BASIC
Stores the GLBuffer (the GL resource) which this pass will render into, as well as the expressions that determine the width/height (both the raw std::string as well as the evaluated expression, capable of being executed with substitutions for variables) and the evaluated value.  Also caches the uniform locations for this target buffer's attributes in the compiled GLSL program.
*/
class VVISF_EXPORT ISFPassTarget	{
	...
	public:
		...
		void evalTargetSize(const VVGL::Size & inSize, std::map<std::string, double*> & inSymbols, const bool & inResize, const bool & inCreateNewBuffer);
